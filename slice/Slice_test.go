...
func Test10(t *testing.T) {
	buf := New[byte]()
	sbuf := pr.Rand[[]byte](pr.TypeHex, humanize.MByte)
	if n, e := io.Copy(buf, bytes.NewReader(sbuf)); e != nil {
		t.Fatal(e)
	} else if n != humanize.MByte {
		t.Fatal()
	} else if !bytes.Equal(sbuf, buf.buf) {
		t.Fatal()
	}
}
...
func Test11(t *testing.T) {
	...
	buf := New[byte]()
	sbuf := pr.Rand[[]byte](pr.TypeHex, humanize.MByte)
	buf.ReadFrom(bytes.NewReader(sbuf))
	tmp := make([]byte, humanize.MByte/2)

	ctx, can := context.WithCancel(context.Background())
	if n, _ := buf.ReadCtx(ctx, tmp); n > 0 {
